...
if ( ! function_exists( 'blank_theme_library_get_all_fonts' ) ) :
/**
...
function blank_theme_library_get_all_fonts() {
	 ...
	 */
	return apply_filters( 'blank_theme_library_all_fonts', array_merge( $heading1, $standard_fonts, $heading2, $google_fonts ) );
}
endif;

if ( ! function_exists( 'blank_theme_library_get_font_choices' ) ) :
/**
 * Packages the font choices into value/label pairs for use with the customizer.
 *
 * @since  1.0.0.
 ...
 * @return array    The fonts in value/label pairs.
 */
function blank_theme_library_get_font_choices() {
	$fonts   = blank_theme_library_get_all_fonts();
	$choices = array();
...
if ( ! function_exists( 'blank_theme_library_get_google_font_uri' ) ) :
/**
 ...
 */
function blank_theme_library_get_google_font_uri( $key, $default , $variant_key, $subset_key ) {

	$font = trim(get_theme_mod( $key, $default ));

	// De-dupe the fonts
	$allowed_fonts = blank_theme_library_get_google_fonts();
	$variant       = get_theme_mod( $variant_key , 'regular' );
	$subset        = get_theme_mod( $subset_key, 'latin' );
	$family        = array();

	// Build the family name and variant string (e.g., "Open+Sans:regular,italic,700")
	if ( ! array_key_exists( $font, $allowed_fonts ) ) return;

	$family = urlencode( $font . ':' . $variant );
	...
	$subsets = ($subset !== 'latin' ) ? array( 'latin', $subset ) : array('latin');
...
if( ! function_exists( 'blank_theme_get_google_fonts_variants' ) ) :

/**
 * Retrieves the font variant list
 * @param  [string] $key  theme_mod key of font
 * @param  [string] $default default font
 * @return [array] variant array
 */
function blank_theme_get_google_fonts_variants( $key , $default , $font_name = false )
{
	$font = $font_name ? $font_name : get_theme_mod( $key , $default );

	$fonts = blank_theme_library_get_google_fonts();
...
if( ! function_exists( 'blank_theme_get_google_fonts_subsets' ) ) :
 ...
 * @return [array] variant array
 */
function blank_theme_get_google_fonts_subsets( $key , $default, $font_name = false )
{
	$font = $font_name ? $font_name : get_theme_mod( $key , $default );
...
if ( ! function_exists( 'blank_theme_library_get_standard_fonts' ) ) :
/**
...
if ( ! function_exists( 'blank_theme_library_get_font_stack' ) ) :
/**
 ...
 */
function blank_theme_library_get_font_stack( $font ) {

	$all_fonts = blank_theme_library_get_all_fonts();

	// Sanitize font choice
	$font = blank_theme_library_sanitize_font_choice( $font );
...
if ( ! function_exists( 'blank_theme_library_sanitize_font_choice' ) ) :
/**
 ...
 */
function blank_theme_library_sanitize_font_choice( $value ) {
	if ( is_int( $value ) ) {
		// The array key is an integer, so the chosen option is a heading, not a real choice
		return '';
	} else if ( array_key_exists( $value, blank_theme_library_get_font_choices() ) ) {
		return $value;
	} else {
		return '';
	}
...
if ( ! function_exists( 'blank_theme_library_get_google_fonts' ) ) :
/**
 ...
 */
function blank_theme_library_get_google_fonts() {
	return apply_filters( 'blank_theme_library_get_google_fonts', array(
		'ABeeZee' => array(
			'label'    => 'ABeeZee',
			'variants' => array(
				'regular',
